...
if (!VK.Widgets) {
  ...
  VK.Widgets.Like = function(objId, options, page) {
    ...
    var 
      ...
      width = type == 'full' ? 350 : (type == 'button' ? 180 : (type == 'mini' ? 100 : 41)),
        ...
        initTooltip: function (counter) {
          tooltipRpc = new fastXDM.Server({
            ...
            statsBox: function (act) {
              ...
              scrollTop = Math.max(parseInt(window.pageYOffset) || 0, document.documentElement.scrollTop,  document.body.scrollTop),
          ...
          tooltipIfr = tooltipRpc.append(obj, {
            src: buttonIfr.src + '&act=a_share_tooltip',
            scrolling: 'no',
            allowTransparency: true,
            id: buttonIfr.id + '_tt',
            style: {position: 'absolute', left: type == 'vertical' || type == 'mini' ? '-36px' : '-2px', padding: 0, display: 'block', visibility: 'hidden', border: '0', width: '206px', height: '127px', zIndex: 140, overflow: 'hidden'}
          });
    ...
    function showTooltip(force) {
      ...
      var scrollTop = document.body.scrollTop || document.documentElement.scrollTop || 0, objPos = VK.Util.getXY(obj)[1];
      if (scrollTop > objPos - 120 || type == 'vertical') {
        tooltipIfr.style.top = (height + 2) + 'px';
        tooltipRpc.callMethod('show', false);
      } else {
        tooltipIfr.style.top = '-125px';
        tooltipRpc.callMethod('show', true);
      }
      tooltipIfr.style.display = 'block';
      tooltipIfr.style.visibility = 'visible';
      isOver = true;
    };
  ...
  VK.Widgets.Auth = function(objId, options, u_key) {
    if (!options.width) {
      options.width = 200;
    }
    VK.Widgets._constructor('widget_auth.php', objId, options, {key: u_key});
  }
  ...
  VK.Widgets._constructor = function(widgetUrl, objId, options, params, funcs, defaults) {
    ...
    if (!params.app) params.app = VK._apiId;
    var url = base_domain + '/' + widgetUrl + '?width=' + width;
    for (i in params) {
