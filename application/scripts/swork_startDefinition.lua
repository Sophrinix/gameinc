...
function AddStartPublicTechnology()
	...
	local tech = CLuaTech( app:AddTechnology( PT_GENRE ) )
	...
	local sndTech = CLuaTech( app:AddTechnology( PT_SOUND ) )
	...
	local videoTech = CLuaTech( app:AddTechnology( PT_VIDEO ) )
	videoTech:SetTechType( VIDEO_TEXT_OUTPUT )
	videoTech:SetName( "Вывод графики в виде текста" )
	videoTech:SetBaseCode( 0.4 )
	videoTech:SetAddingEngineCode( 0.2 )
	videoTech:SetEngineTechRequire( VIDEO_TEXT_OUTPUT, 10 )
	...
	local advTech( CLuaTech( app:AddTechnology( PT_ADVANCED ) )
