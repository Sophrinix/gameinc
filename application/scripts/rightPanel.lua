...
local function _Resort( sender )
	if _childCount == _mainWindow.childCount then
		return
	end
...
function AddYesNo( text, funcYes, funcNo )
	local wnd = guienv:AddWindow( "rightPanelWindow.png", 0, 0, "0e", _defaultHeight.."+", -1, _mainWindow )
...
function AddOk( text, funcOk )
	local wnd = guienv:AddWindow( "rightPanelWindow.png", 0, 0, "311+", "154+", -1, _mainWindow )
	local lb = guienv:AddLabel( text, "5%", "5%", "80%", "60%", -1, wnd )
	wnd.drawBody = false
...
function Show()
	if _mainWindow == nil then
		...
		_childCount = _mainWindow.childCount
		guienv:AddTopElement( _mainWindow )
		guienv:AddLoopTimer( 1000, _Resort, _mainWindow )
