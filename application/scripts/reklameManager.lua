...
function ApplyNewWork()
	...
	--снимем деньги со счета компании дл€ проведени€ рекламной акции
...
function QuerryUserToApplyWork()
	if addingDays > 0 then
		...
		local text = "–екламна€ кампани€:"..currentWork:GetName().."\n"
		text = text .. "—тоимость:" .. currentWork:GetDayCost() * currentWork:GetNumberDay() .. "\n"
		text = text .. "ƒлительность:" .. currentWork:GetNumberDay()
		guienv:MessageBox( text, true, true, "./reklameManager.ApplyNewWork()", "./reklameManager.CancelNewWork()" )
	else
		guienv:MessageBox( "¬ыберите количество дней кампании", false, false, "", "" )
	end
end
