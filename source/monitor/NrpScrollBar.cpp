...
CNrpScrollBar::CNrpScrollBar(bool horizontal, IGUIEnvironment* environment,
...
bool CNrpScrollBar::OnEvent(const SEvent& event)
{
	if (IsEnabled)
	{
...
void CNrpScrollBar::OnPostRender(u32 timeMs)
{
...
void CNrpScrollBar::draw()
{
	...
	if( texture_ )
	{
		Environment->getVideoDriver()->draw2DImage( texture_, AbsoluteRect, textureRect_, &AbsoluteClippingRect, 0, true );
	}
	else
	...
	if ( core::isnotzero ( range() ) )
	{
		...
		if( sliderTexture_ )
		{
			Environment->getVideoDriver()->draw2DImage( sliderTexture_, SliderRect, sliderTextureRect_, &AbsoluteClippingRect, 0, true);
		}
		else
...
void CNrpScrollBar::updateAbsolutePosition()
{
...
//!
s32 CNrpScrollBar::getPosFromMousePos(const core::position2di &pos) const
{
	f32 w, p;
	if (Horizontal)
	{
...
void CNrpScrollBar::setPos(s32 pos)
{
...
s32 CNrpScrollBar::getSmallStep() const
{
	return SmallStep;
}
...
void CNrpScrollBar::setSmallStep(s32 step)
{
	if (step > 0)
		SmallStep = step;
	else
...
//! gets the small step value
s32 CNrpScrollBar::getLargeStep() const
{
	return LargeStep;
}
...
//! sets the small step value
void CNrpScrollBar::setLargeStep(s32 step)
{
	if (step > 0)
		LargeStep = step;
	else
...
s32 CNrpScrollBar::getMax() const
{
	return Max;
}
...
void CNrpScrollBar::setMax(s32 max)
{
...
//! gets the maximum value of the scrollbar.
s32 CNrpScrollBar::getMin() const
{
	return Min;
}
...
void CNrpScrollBar::setMin(s32 min)
{
...
s32 CNrpScrollBar::getPos() const
{
	return Pos;
}
...
void CNrpScrollBar::refreshControls()
{
	...
	if (Horizontal)
	{
		...
		if (!UpButton)
		{
			UpButton = Environment->addButton( core::rect<s32>(0,0, h, h), this, -1 );
		...
		if (!DownButton)
		{
			DownButton = Environment->addButton( core::rect<s32>(RelativeRect.getWidth()-h, 0, RelativeRect.getWidth(), h), this, -1 );
	...
	else
	{
		...
		if (!UpButton)
		{
			UpButton = Environment->addButton( core::rect<s32>(0,0, w, w), this, -1 );
			UpButton->setSubElement(true);
			UpButton->setTabStop(false);
		}
		if (sprites)
		...
		if (!DownButton)
		{
			DownButton = Environment->addButton( core::rect<s32>(0,RelativeRect.getHeight()-w, w, RelativeRect.getHeight()), this, -1 );
			DownButton->setSubElement(true);
			DownButton->setTabStop(false);
		}
		if (sprites)
...
void CNrpScrollBar::serializeAttributes(io::IAttributes* out, io::SAttributeReadWriteOptions* options=0) const
{
...
void CNrpScrollBar::deserializeAttributes(io::IAttributes* in, io::SAttributeReadWriteOptions* options=0)
{
...
void CNrpScrollBar::setTexture( video::ITexture* texture )
{
	if (texture_)
		texture_->drop();
	...
	texture_ = texture;
	if (texture)
		textureRect_ = core::rect<s32>(core::position2d<s32>(0,0), texture->getOriginalSize());

	if (texture_)
		texture_->grab();
}

void CNrpScrollBar::setSliderTexture( video::ITexture* texture )
{
	if ( sliderTexture_)
		sliderTexture_->drop();

	sliderTexture_ = texture;
	if( texture )
		sliderTextureRect_ = core::rect<s32>(core::position2d<s32>(0,0), texture->getOriginalSize());
