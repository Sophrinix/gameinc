...
public:
		...
		Environment(environment), Type(type), AlphaBlend(0xff), Name(L"")
	{
		...
		if (parent)
		{
			...
			AlphaBlend = parent->getAlphaBlend();
		}
	}
	...
	*/
	...
	\return Returns the first element with the given id. If no element
	with this id was found, 0 is returned. */
	virtual IGUIElement* getChildenFromName(const core::stringw& name, bool searchchildren=false) const
	{
		IGUIElement* e = 0;
		core::list<IGUIElement*>::ConstIterator it = Children.begin();
		for (; it != Children.end(); ++it)
		{
			if( name.equals_ignore_case( (*it)->getName() ) )
			 	 return (*it);
			if (searchchildren)
				e = (*it)->getChildenFromName(name, true);
			if (e)
				return e;
		}
		return e;
